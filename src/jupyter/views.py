...
class JupyterSessionViewSet(viewsets.ModelViewSet):
    ...
    def _get_user_token(self, uid):
        ...
        fernet = Fernet(base64.b64encode(JUPYTER_ADMIN_TOKEN.encode("utf-8")))
    ...
    def _get_jupyter_server_spawn_progress(self, token):
        ...
        response = requests.get(url, headers=self.jupyter_headers, stream=True)
    ...
    def create(self, request, *args, **kwargs):
        ...
        session, _ = JupyterSession.objects.get_or_create(uid=uid, filename=filename)
    ...
    def start_jupyter_user_server(self, request, token=None):
        ...
        try:
            ...
            if server_starting:
                for event in self._get_jupyter_server_spawn_progress(token):
                    print(event)
                ...
                return Response(
                    {"data": "Server is running", "user": created}, status=200
                )
    ...
    def create_or_get_jupyterhub_session(self, request, token=None):
        ...
        try:
            if created:
                ...
                url = (
                    f"{BASE_JUPYTER_URL}/hub/user/{token}/api/contents/{filename}.ipynb"
                )
                response = requests.get(url, headers=self.jupyter_headers)
            ...
            data = {"data": str(e)}
        return Response({"data": data, "status_code": response.status_code}, status=200)
